...
enum endPoints {
    ...
    case genreMovieList(api_key : String, language : String)
    case genreMovies(api_key : String, genre_id : String, language : String)
    ...
    func provideValues() -> (url : String, parameters : Parameters, httpMethod : HTTPMethod, parseCode : Int) {
        ...
        case .genreMovieList(let api_key, let language):
            let url = BASE_URL + GENRE_MOVIE_LIST_API
            let params = parameters(api_key: api_key, language: language)
            return (url : url, parameters : params, httpMethod : HTTPMethod.get, parseCode : 6)
        case .genreMovies(let api_key, let genre_id, let language):
            let url = BASE_URL + "/genre/" + genre_id + "/movies"
            let params = parameters(api_key: api_key, language: language)
            return (url : url, parameters : params, httpMethod : HTTPMethod.get, parseCode : 0)
